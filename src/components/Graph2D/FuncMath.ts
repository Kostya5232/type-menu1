...
export default class FuncMath {
    ...
    constructor(funcMath: { WIN: TWIN2D; canvas: Canvas }) {
        this.WIN = funcMath.WIN;
        this.canvas = funcMath.canvas;
    }

    getDerivative(f: Function, x0: number, dx = 0.00001) {
    ...
    getIntegral(f: Function, a: number, b: number, n = 100) {
        const dx = (b - a) / n;
        let x = a;
        let s = 0;
        while (x < b) {
    ...
    getZero(f: Function, a = 0, b = 0, eps = 0): number | null | undefined {
        if (f(a) * f(b) > 0) {
            return null;
        }
    ...
    getCross(f: Function, g: Function, a = 0, b = 0, eps = 0): number | null | undefined {
